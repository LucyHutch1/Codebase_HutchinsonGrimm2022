...
function visualize_balls_2D_blank(mySystem)
    ...
    ytu = mod(mySystem.TU.TUcells,mySystem.grid.N); %was +0
    %xtu = mySystem.TU.TUcells/(mySystem.grid.N-1));   %was -1 *this is the original*

    xtu = ceil(double(mySystem.TU.TUcells)/(mySystem.grid.N));   %was -1
    %the one above seems to be working
    %xtu = ceil(mySystem.TU.TUcells/(mySystem.grid.N));   %was -1 
 ...
 %   xim = ceil(mySystem.IM.IMcells/mySystem.grid.N); (original)
    xim = ceil(double(mySystem.IM.IMcells)/mySystem.grid.N);
    ...
    text(40,-10,[num2str(mySystem.grid.StepsDone/2),' days'],...
        'Color','k','FontWeight','bold','FontSize',18,'VerticalAlignment','top')
