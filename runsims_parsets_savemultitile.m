...
%
function [] = runsims_parsets_savemultitile(pars_all, par_names, pats_chosen, endtimes, sim_endtimes,...
    tilenumbers, tile_limit,  n_stoch, save_path, task_name,savetimecourse,saveselectedgridstack)%

n_pars = length(pars_all);

%% Run simulations for chosen patients for every parameter sample and save the full outputs

for p = 1:length(pats_chosen)
    pat = pats_chosen(p);
    obs_endTime = endtimes(p); % Observed timepoint on treatment biopsy
    sim_endTime = sim_endtimes(p); % Time to which we wish to simulate 
    n_tiles_curr = tilenumbers(p);
    
    % For the patients with more than tile_limit tiles, we want to
    % subsample tile_limit tiles at random to improve computation time and avoid massive
    ...
    % Set up a vector of the instance to save [pat, stoch] if
    % applicable
    instance_to_save = [];
    if ~isempty(saveselectedgridstack)
            
        if ismember(pat,saveselectedgridstack(:,1))
            % Extract the row of details of what to save (in this case, 2
            % timepoints, all tiles, all pars
            instance_to_save = saveselectedgridstack(saveselectedgridstack(:,1)==pat,:);

            if instance_to_save(2)>n_stoch
                instance_to_save(2)=n_stoch;
            end
            if instance_to_save(3)>sim_endTime
                instance_to_save(3)=sim_endTime/2;
            end
            if instance_to_save(4)>sim_endTime
                instance_to_save(4)=sim_endTime;
            end       
        end
   ...
   filename_preStruct = ['final_tiles/PatientData/p',num2str(pat),'/','pre','_',...
    ...
    simout_selected = cell(n_pars,n_tiles_sim); % for saving one chosen instance
    ...
    parfor j = 1:n_pars % Parameter set loop
        ...
        SimOut = cell(n_stoch,n_tiles_sim);
        ...
        for tt = 1:n_tiles_sim
            for stoch = 1:n_stoch
                ...
                if ~isempty(instance_to_save)
                    if instance_to_save(1) == pat && instance_to_save(2) == stoch
                        if length(SimOut{stoch,tt})>= instance_to_save(3)
                            simout_selected{j,tt}{1} = SimOut{stoch,tt}{instance_to_save(3)};
                        else
                            simout_selected{j,tt}{1} = [];
                        end
                        if length(SimOut{stoch,tt})>= instance_to_save(4)
                            simout_selected{j,tt}{2} = SimOut{stoch,tt}{instance_to_save(4)};
                        else
                            simout_selected{j,tt}{2} = [];
                        end
                    end
                else
                    simout_selected{j,tt} = [];
                end
