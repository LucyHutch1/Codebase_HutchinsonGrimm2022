...
%
function [h]=plot_timecourses_test(currpat, data_summary,p, q,n_CD8_observed_all, n_CD8_observed_PRE_all, set_ind, pat, directory,h)
...
CD8_timecourse_mean_curr = nanmean(currpat.CD8_timecourse_all,3);
        ...
        plot(ones(size(n_CD8_observed_all{q}))*data_summary.endTimes(q),n_CD8_observed_all{q},'b*')
        % Add CD8S at the start as well
        plot(zeros(size(n_CD8_observed_PRE_all{q})),n_CD8_observed_PRE_all{q},'b*')
        ...
        title(['Patient ',num2str(pat),', CD8 timecourse predictions: parameter set ',title_tag])
        mkdir(directory)
        print([directory,'/TimecourseCD8_simple_median_iqr',tag,'_pat',num2str(pat)],'-dpng')
